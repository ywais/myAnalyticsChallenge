...
router.get('/all-filtered', (req: Request, res: Response) => {
  ...
  if (filters.search) {
    ...
    events = events.filter((event) => Object.keys(event).map((key) => {
      if (typeof event[key] === 'string') {
        return !!regex.test(event[key] as string);
      }
      return false;
    }).includes(true));
  }
  if (filters.sorting) {
    events.sort((firstEvent: Event, secondEvent: Event) => (filters.sorting === '-date'
...
router.get('/by-days/:offset', (req: Request, res: Response) => {
  ...
  res.json(weeklySessions.map((day) => ({
    date: new Date(day.date).toDateString(), count: day.uniqueSessions.length,
  })));
});
...
router.get('/by-hours/:offset', (req: Request, res: Response) => {
  ...
  const addUniqueSessions = (hour: number, sessionId: string) => {
    if (dailySessions[hour].uniqueSessions.length === 0
      || !dailySessions[hour].uniqueSessions.includes(sessionId)) {
...
router.get('/chart/pageview/', (req: Request, res: Response) => {
  ...
  events.forEach((event) => {
    ...
    index >= 0
      ...
      : pageViews.push({ name: event.url.substring(21), views: 1 });
  });
  res.json(pageViews);
});
