...
const ChartEventsLog: React.FC = () => {
  ...
  const displayObject = (objectToDisplay: Event): JSX.Element[] =>
    Object.keys(objectToDisplay).map((key: string): JSX.Element => {
      if(key === "geolocation") {
        return <div key={key}>
          <Typography className={classes.span}>
            <b>{key}:</b> {JSON.stringify(objectToDisplay[key]).replaceAll('"','').replaceAll(':',': ')}
          </Typography>
        </div>;
      } if(key === "date") {
        return <div key={key}>
          <Typography className={classes.span}><b>{key}:</b> {new Date(objectToDisplay[key]).toDateString()}</Typography>
        </div>;
      }
      return <div key={key}><Typography className={classes.span}><b>{key}:</b> {objectToDisplay[key]}</Typography></div>;
    });
        ...
        <div key={"event " + index} className="eventInLog">
          <Accordion>
            ...
            >
              ...
              />
              <Typography className={classes.heading}> {event.name} event from {new Date(event.date).toDateString()}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <img
